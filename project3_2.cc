...
//配置AP移动方式，ConstantPositionMobilityModel，固定位置模型
 ...
 //客户端3通信Echoudp通信
  ...
  if (tracing == true)
    {
      pointToPoint.EnableAsciiAll (ascii.CreateFileStream("project3_2_pointopoint.tr"));
      pointToPoint.EnablePcapAll ("third");

      phy.EnablePcap ("third", apDevices.Get (0));
      phy.EnableAscii (ascii.CreateFileStream("project3_2_phy.tr"),apDevices.Get (0));

      csma.EnablePcap ("third", csmaDevices.Get (0), true);
      csma.EnableAscii(ascii.CreateFileStream("project3_2_csma.tr"),csmaDevices.Get (0));
